...
/**
  ...
  * @retval int
  */
static uint32_t value1 = 0, value2 = 0;
static uint8_t printa = 0;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  printa = 1;
    ...
    if (GPIO_Pin == GPIO_PIN_1) // your pin
    {
        if (first_capture)
        {
        ...
        else
        {
            ...
            // Now value2 = time period in ticks
        }
    }
}
...
HAL_TIM_Base_Init(&htim2);
  ...
  while (1)
  {
    ...
    //   HAL_UART_Transmit(&huart1, (uint8_t*)buffer, 50, HAL_MAX_DELAY);
    //   print_flag = 0;
    // }
      ...
      if(printa)
      {
          ...
          int len = sprintf(buffer, "Frequency: %lu Hz\r\n", freq);
